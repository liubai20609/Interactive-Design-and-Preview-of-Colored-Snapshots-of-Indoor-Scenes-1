...
class abGrid():
    def __init__(self, gamut_size=110, D=1):
        ...
        # self.Tongueimg = np.array(skimage.io.imread('./test_imgs/100Color.bmp')).astype('uint8')
        self.Tongueimg = np.array(skimage.io.imread('./test_imgs/seka1.bmp')).astype('uint8')
    ...
    def update_gamut(self, l_in):
        ...
        # l_in=50 ###
        if l_in==25:
            self.Tongueimg = np.array(skimage.io.imread('./test_imgs/timg1.bmp')).astype('uint8')
        elif l_in==50:
            self.Tongueimg = np.array(skimage.io.imread('./test_imgs/gray.bmp')).astype('uint8')
        elif l_in==75:
            self.Tongueimg = np.array(skimage.io.imread('./test_imgs/seka1.bmp')).astype('uint8')
        ...
        self.mask = pts_lab_diff > -1  #20190530

        mask3 = np.tile(self.mask[..., np.newaxis], [1, 1, 3])
        self.masked_rgb = self.pts_rgb.copy()
        #yanhai_begin
        self.masked_rgb = self.Tongueimg.copy() #20190530



        # self.masked_rgb[np.invert(mask3)] = 255 #20190530
        #begin_end
    ...
    def findPosTongueImg(self, x, y):
        PosColor = self.Tongueimg[x, y, :]
        # r = self.Tongueimg[x,y,0]
        # g = self.Tongueimg[x,y,1]
        # b = self.Tongueimg[x,y,2]
        return PosColor

    def changeGamut(self,ll):
        if ll>50:
            self.Tongueimg = np.array(skimage.io.imread('./test_imgs/timg1.bmp')).astype('uint8')

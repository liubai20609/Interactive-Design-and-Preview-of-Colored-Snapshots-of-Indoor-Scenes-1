...
class GUIDesign(QWidget):
    def __init__(self, color_model, dist_model=None, img_file=None, load_size=256,
        ...
        gamutLayout = self.AddWidget(self.gamutWidget, 'Color Gamut')
        ...
        # palette
        ##yanhai_begin 增加转换色系的Button
        # self.changeStyle = gui_palette.GUIPalette(grid_sz=(10, 1))
        # changeStyleBtn = self.AddWidget(self.changeStyle, 'ChangeColorStyle')
        # colorLayout.addLayout(changeStyleBtn)
        self.ChangeColorBtn = QPushButton('ChangeColorStyle')  # to visualize the selected color
        self.ChangeColorBtn.setFixedWidth(321)
        self.ChangeColorBtn.setFixedHeight(25)
        self.ChangeColorBtn.setStyleSheet("background-color: grey")
        ChangeColorStyle = self.AddWidget(self.ChangeColorBtn, 'ChangeColorStyle')
        colorLayout.addLayout(ChangeColorStyle)
        ...
        self.updateGamut = lab_gamut.abGrid(gamut_size=110, D=1)
        ...
        self.bOK.setToolTip('Colorization OK')
        # yanhai_end
        ...
        drawPadMenu.addWidget(self.bOK)
        ...
        self.ChangeColorBtn.clicked.connect(self.ChangeColorStyle)
    ...
    def OK(self):
        ...
        result = self.drawWidget.okPress()
        # self.drawWidget.okImageE(result)

        # self.OK()
        # self.OK=gui_gamut.GUIGamut.canOK
        # if True ==ok and gui_gamut.GUIGamut.is_valid_point(pos=gui_gamut.GUIGamut.getPos):
        if True == self.canOK:
            print("okokle")
            # gui_gamut.GUIGamut.update_ui()
        else:
            print("NO_OK!!!")

    def ChangeColorStyle(self):
        print('*********ChangeColorStyle***********')
        self.ll = (self.ll+25)%100
        if self.ll == 0:
            self.ll =25
        print(self.ll)
        # self.updateGamut.changeGamut(self.ll)
        self.gamutWidget.set_gamut(l_in=self.ll)
        # self.gamutWidget.set_ab([0,255,0])
        # self.drawWidget.set_color([0,255,0])
        # gui_gamut.GUIGamut.set_gamut(l_in=self.ll)
